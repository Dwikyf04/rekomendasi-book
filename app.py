...
@st.cache_data
def load_dataframe(file_path="data - books.csv"): # <-- DIUBAH DARI .pkl ke .csv
    """
    Memuat data buku MENTAH dari file .csv dan memprosesnya.
    """
    try:
        df = pd.read_csv(file_path) # <-- DIUBAH ke pd.read_csv
        
        # --- PREPROCESSING (PENTING) ---
        # Kode ini sekarang berjalan setiap kali cache data di-refresh
        df = df.drop_duplicates(subset=[c for c in ['isbn13','title'] if c in df.columns], keep='first')
        df['title'] = df['title'].astype(str)
        
        # Buat kolom 'text' gabungan (KOLOM KUNCI UNTUK MODEL)
        cols = []
        ...
        if cols:
            ...
            # Fallback jika kolom-kolom itu tidak ada
            st.warning("Kolom 'title', 'authors', 'description' dll tidak ditemukan. Menggunakan 'title' saja.")
            # Pastikan 'title' ada dan diisi
            if 'title' in df.columns:
                df['text'] = df['title'].fillna('') 
            else:
                st.error("Dataset HARUS memiliki setidaknya kolom 'title'.")
                return pd.DataFrame()
        ...
        st.error(f"File data '{file_path}' tidak ditemukan. Pastikan file ada di repositori.")
        return pd.DataFrame()
    except Exception as e:
        st.error(f"Gagal memuat DataFrame dari CSV: {e}")
        return pd.DataFrame()
...
def load_models():
    ...
    (Fungsi ini tidak memuat books_df.pkl)
    """
    models_path = "model/" # Asumsi model ada di root
    try:
        tfidf_vec = joblib.load(os.path.join(models_path, "model/tfidf_vectorizer.pkl"))
        kmeans_model = joblib.load(os.path.join(models_path, "model/kmeans_model.pkl"))
        knn_model = joblib.load(os.path.join(models_path, "model/knn_model.pkl"))
        embeddings = joblib.load(os.path.join(models_path, "model/embeddings.pkl"))
        
        sbert_model = None
        if HAS_SBERT:
            sbert_model = SentenceTransformer('paraphrase-multilingual-MiniLM-L12-v2')
            
        st.success("Semua model (4 file) berhasil dimuat.")
        return tfidf_vec, kmeans_model, knn_model, embeddings, sbert_model
    
    except FileNotFoundError as e:
        st.error(f"File model tidak ditemukan: {e}.")
        st.warning("Pastikan file .pkl (selain books_df.pkl) ada di folder yang benar.")
...
# ----------------------
df_books = load_dataframe("data - books.csv") # <-- DIUBAH
tfidf_vectorizer, kmeans_model, knn_model, sbert_embeddings, sbert_model = load_models()

# ----------------------
# 3. Navigasi Atas (Horizontal) - Tampilan UNAIR
# ----------------------
...
# ===============================================
# Halaman 1: BERANDA (Tampilan UNAIR)
# ===============================================
if selected_page == "Beranda":
    ...
    .icon-button-icon { font-size: 48px; line-height: 1; }
    .icon-button-text { margin-top: 10px; font-weight: bold; font-size: 14px; }
    a:link, a:visited { text-decoration: none !important; color: inherit !important; }
    </style>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div class="icon-button"><div class="icon-button-icon">📅</div><div class="icon-button-text">Latest Additions</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="icon-button"><div class="icon-button-icon">🔎</div><div class="icon-button-text">Advanced Search</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="icon-button"><div class="icon-button-icon">🗂️</div><div class="icon-button-text">Browse Repository</div></div>', unsafe_allow_html=True)
    with col4:
        st.markdown('<div class="icon-button"><div class="icon-button-icon">ℹ️</div><div class="icon-button-text">About us</div></div>', unsafe_allow_html=True)
    with col5:
        st.markdown('<div class="icon-button"><div class="icon-button-icon">📜</div><div class="icon-button-text">Policies</div></div>', unsafe_allow_html=True)
...
# ===============================================
    ...
    if df_books.empty or knn_model is None or sbert_embeddings is None:
        st.error("Model (KNN/Embeddings) atau Data (CSV) tidak tersedia. Fitur ini dinonaktifkan.")
    else:
...
# ===============================================
    ...
    nama_cluster_buku = {
        0: "Fiksi & Sastra", 1: "Teknis & Sains",
        2: "Self-Help & Bisnis", 3: "Biografi & Sejarah"
